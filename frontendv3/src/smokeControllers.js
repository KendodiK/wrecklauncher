...
export async function runSmokeControllers() {
  console.log('[smoke] runSmokeControllers() called');
  ...
  try {
    if (globalThis.__wreck_smoke_ran__ === true) {
      console.log('[smoke] skipping (already ran)');
      return;
    }
  ...
  try {
    ...
    try {
      ...
      log('register', token);
      if (!token) throw new Error('register returned empty');
      token = token.token || token; // handle both raw token or { token } object
    } catch (e2) {
      warn('register', e2);
    }
  }
  ...
  try{
    ...
    try {
      const platformUser = await api.createPlatformUser('steam', 'freshargentinaccount69912', 'testpassword', '76561199194098023');
