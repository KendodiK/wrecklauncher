...
class LibraryCacheController {
  ...
  constructor() {
    ...
    if (!selected) {
      throw new Error('Unable to initialize writable cache directory');
    }

    this.#cachePath = path.join(selected, 'library-installed-cache.json');
  }
  ...
  /**
   * @returns {{ entries: Record<string, { value?: any[], expiresAt?: number, updatedAt?: number }>, updatedAt?: string }}
   */
  #readFile() {
    try {
      ...
      console.warn('[library-cache] Failed to read cache file:', err);
      return { entries: {} };
    }
  }

  /**
   * @param {{ entries: Record<string, { value?: any[], expiresAt?: number, updatedAt?: number }>, updatedAt?: string }} payload
   * @returns {boolean}
   */
  #writeFile(payload) {
    try {
      ...
      console.warn('[library-cache] Failed to write cache file:', err);
      return false;
    }
  }
  ...
  /**
   * @param {any} keyRaw
   * @returns {{ value: any[], expiresAt: number, updatedAt: number }|null}
   */
  get(keyRaw) {
    ...
    return {
      value: entry.value,
      expiresAt: Number.isFinite(expiresAt) ? Math.max(0, Math.trunc(expiresAt)) : 0,
   ...
   * @param {any} keyRaw
   * @param {any} value
   * @param {any} expiresAtRaw
   * @returns {boolean}
   */
  set(keyRaw, value, expiresAtRaw) {
    const key = this.#normalizeKey(keyRaw);
    if (!key) return false;
...
module.exports = LibraryCacheController;
