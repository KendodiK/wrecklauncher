...
// Lazy-load heavy deps (speeds up cold start if you don't hit endpoints using them).
let puppeteer;
...
//#region HTTP/HTTPS Server Setup
/**
 ...
 * 3) cached dev self-signed cert: `./certs/dev-selfsigned-*.pem`
 * 4) dev fallback: generate a self-signed cert (requires `selfsigned` dependency)
 */
function loadHttpsOptions() {
  ...
  // Cached dev self-signed cert to avoid re-generating on every start.
  const cachedKeyPath = path.join(certDir, 'dev-selfsigned-key.pem');
  const cachedCertPath = path.join(certDir, 'dev-selfsigned-cert.pem');
  if (fs.existsSync(cachedKeyPath) && fs.existsSync(cachedCertPath)) {
    return {
      key: fs.readFileSync(cachedKeyPath),
      cert: fs.readFileSync(cachedCertPath),
    };
  }
  ...
  // Save for next start (faster) – these are DEV certs.
  try {
    fs.mkdirSync(certDir, { recursive: true });
    fs.writeFileSync(cachedKeyPath, pems.private);
    fs.writeFileSync(cachedCertPath, pems.cert);
    console.warn('[TLS] Generated and cached a dev self-signed certificate in ./certs (dev-selfsigned-*.pem).');
  } catch {
    console.warn('[TLS] Generated a dev self-signed certificate (not cached).');
  }

  console.warn('[TLS] Self-signed certificates are not trusted by default.');
  console.warn('[TLS] For a trusted dev cert, use mkcert and set SSL_KEY_PATH + SSL_CERT_PATH.');
...
function startServers() {
  ...
  // Optional plain HTTP listener for quick local testing / debugging.
  // (Keep it enabled by default; disable by setting HTTP_PORT=0)
  if (httpPort > 0) {
  ...
  if (httpPort > 0) console.log(`API server (HTTP) running at http://localhost:${httpPort}`);
}
